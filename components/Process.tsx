    ...
    <section className={`py-28 bg-[#fafaf8] relative overflow-hidden ${isRtl ? "rtl" : "ltr"}`}>
      ...
      <div className="relative max-w-6xl mx-auto px-6">
        ...
        <div className="relative grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* timeline line */}
          <div className="hidden lg:block absolute top-8 left-[16.66%] right-[16.66%] h-px bg-gradient-to-r from-sky-200 via-violet-200 to-emerald-200" />
              ...
              <motion.div key={i}
                ...
                {i < steps.length - 1 && (
                  <div className={`hidden lg:flex absolute top-8 ${isRtl ? "-left-3" : "-right-3"} z-10 w-6 h-6 items-center justify-center`}>
                    <Arrow size={14} className="text-slate-300" />
                  </div>
                )}
              </motion.div>
            );
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          <a href={`https://wa.me/972543495645`} target="_blank" rel="noopener noreferrer"
            className="btn-beam inline-flex items-center gap-2.5 bg-slate-900 hover:bg-slate-800 text-white font-black px-8 py-4 rounded-2xl transition-all hover:scale-[1.03] shadow-xl text-base">
